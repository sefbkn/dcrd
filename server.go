...
func (s *server) handleAddPeerMsg(state *peerState, sp *serverPeer) bool {
	...
	peerIPBytes := sp.NA().IP
	peerNetIP := net.IP(peerIPBytes)
	if cfg.MaxSameIP > 0 && !isInboundWhitelisted && !peerNetIP.IsLoopback() &&
		state.ConnectionsWithIP(peerIPBytes)+1 > cfg.MaxSameIP {
...
func newPeerConfig(sp *serverPeer) *peer.Config {
	...
	return &peer.Config{
		...
		HostToNetAddress: func(host string, port uint16, services wire.ServiceFlag) (*addrmgr.NetAddress, error) {
			return sp.server.addrManager.HostToNetAddress(host, port,
				addrmgr.ServiceFlag(services))
		},
		Proxy:             cfg.Proxy,
